...
@AndroidEntryPoint
class EditPostFragment : Fragment(), MentionPeopleItemListener {
    ...
    private fun adapterControl() {
        mentionAdapter = MentionPeopleAdapter(mentionedLogins, this@EditPostFragment)
    ...
    private fun addPost() {
        binding.add.setOnClickListener {
            postVM.updateFromExternalStorage(
            ...
            postVM.updateById(
                updatedPost.copy(mentionIds = mentionedIds),
    ...
    private fun postControl() = lifecycleScope.launchWhenCreated {
        postId = arguments?.editedPostId!!.toInt()
        if (postId != 0) {
